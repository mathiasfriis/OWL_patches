  ...
  void processAudio(AudioBuffer &buffer){
    ...
    else
    {
    	//calculate relative sample rate
   		...
   		relativeSampleRate = pow(1/(eg.getValue()+1),5) * depth * (1-FS_OFFSET); //Gives an exponential-like movement, so that movement around lower sample freqs are more fine, and more course in higher.
    }
